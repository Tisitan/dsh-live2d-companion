...
🎮 独立功能钮直达对局：挂件模式为非模态浮卡；桌宠模式弹**独立卫星小窗**（`game-card.html`，与透明 overlay 物理隔离，下棋时输入不串扰）；独立版同样走卫星窗，网页环境保留页内浮卡回退。

> **焦点如实说明**（更正旧表述「不抢前台焦点」）：卫星窗自 7becb95 起为 `focusable:true`——下拉/输入原生可用所必需，**点击卡片会正常取得前台焦点**，属预期行为；透明 overlay 桌宠窗则经 2026-09-24 win32 修复后不再抢前台（`focusable:false` + 打开输入面板时动态放行）。
